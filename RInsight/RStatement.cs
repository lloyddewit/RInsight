...
///             have the same precedence.</summary>
    ...
    public RStatement(List<RToken> lstTokens, ref int iPos, Dictionary<string, RStatement> dctAssignments)
    {
        ...
        while (iPos < lstTokens.Count)
        {
            ...
            if (lstTokens[iPos].tokentype == RToken.typToken.REndStatement || lstTokens[iPos].tokentype == RToken.typToken.REndScript) // we don't add this termination condition to the while statement 
        ...
        if (lstTokenTree[0].tokentype == RToken.typToken.ROperatorBinary && lstTokenTree[0].childTokens.Count > 1)
        {
        ...
        if (!(clsAssignment == null))
        {
            ...
            strAssignmentPrefix = lstTokenTree[0].childTokens[0].tokentype == RToken.typToken.RPresentation ? lstTokenTree[0].childTokens[0].text : "";
        }
        ...
        if (clsTokenEndStatement.tokentype == RToken.typToken.REndStatement || clsTokenEndStatement.tokentype == RToken.typToken.REndScript)
        {
            ...
            {
                strSuffix = clsTokenEndStatement.childTokens.Count > 0 && clsTokenEndStatement.childTokens[0].tokentype == RToken.typToken.RPresentation ? clsTokenEndStatement.childTokens[0].text : "";
                ...
                default:
                    {
                        ...
                        {
                            clsToken.childTokens.Add(new RToken(strPrefix, RToken.typToken.RPresentation));
                        }
...
/// --------------------------------------------------------------------------------------------
    private List<RToken> GetLstTokenFunctionBrackets(List<RToken> lstTokens)
    {
        ...
        while (iPos < lstTokens.Count)
        {
            ...
            if (clsToken.tokentype == RToken.typToken.RFunctionName)
            {
...
/// --------------------------------------------------------------------------------------------
    private List<RToken> GetLstTokenOperatorGroup(List<RToken> lstTokens, int iPosOperators)
    {
        ...
        while (iPosTokens < lstTokens.Count)
        {
            ...
            if ((operatorPrecedences[iPosOperators].Contains(clsToken.text) || iPosOperators == iOperatorsUserDefined && Regex.IsMatch(clsToken.text, "^%.*%$")) && (clsToken.childTokens.Count == 0 || clsToken.childTokens.Count == 1 && clsToken.childTokens[0].tokentype == RToken.typToken.RPresentation))
            {
            ...
            case RToken.typToken.RFunctionName:
                {
                    ...
                    foreach (var clsTokenParam in clsToken.childTokens[clsToken.childTokens.Count - 1].childTokens)
                    {
                        ...
                        if (clsTokenParam.tokentype == RToken.typToken.RPresentation)
                        {
                            if (bFirstParam)
                            {
                                continue;
                        ...
                        case "$":
                            {
                                ...
                                int startPos = clsToken.childTokens[0].tokentype == RToken.typToken.RPresentation ? 1 : 0;
                                for (int iPos = startPos, loopTo = clsToken.childTokens.Count - 2; iPos <= loopTo; iPos++)
                                {
                                    ...
                                    if (iPos == startPos && clsTokenObject.tokentype == RToken.typToken.ROperatorBinary && clsTokenObject.text == "::")
                                    {
...
/// --------------------------------------------------------------------------------------------
    private string GetPackagePrefix(RToken clsToken)
    {
        ...
        return clsTokenPackageName.childTokens.Count > 0 && clsTokenPackageName.childTokens[0].tokentype == RToken.typToken.RPresentation ? clsTokenPackageName.childTokens[0].text : "";
    }
            ...
            case "=":
                {
                    ...
                    // will be lost.
                    clsParameter.strPrefix = clsTokenArgumentName.childTokens.Count > 0 && clsTokenArgumentName.childTokens[0].tokentype == RToken.typToken.RPresentation ? clsTokenArgumentName.childTokens[0].text : "";
            ...
            default:
                {
                    ...
                    clsParameterNamed.strPrefix = clsToken.childTokens.Count > 0 && clsToken.childTokens[0].tokentype == RToken.typToken.RPresentation ? clsToken.childTokens[0].text : "";
...
/// --------------------------------------------------------------------------------------------
    private static RToken GetChildPosNonPresentation(RToken clsToken)
    {
        ...
        foreach (var clsTokenChild in clsToken.childTokens)
        {
            ...
            if (!(clsTokenChild.tokentype == RToken.typToken.RPresentation) && !(clsTokenChild.tokentype == RToken.typToken.RBracket && clsTokenChild.text == ")"))
            {
...
/// --------------------------------------------------------------------------------------------
    private RParameter GetRParameter(RToken clsToken, Dictionary<string, RStatement> dctAssignments)
    {
        ...
        return new RParameter()
        {
            ...
            strPrefix = clsToken.childTokens.Count > 0 && clsToken.childTokens[0].tokentype == RToken.typToken.RPresentation ? clsToken.childTokens[0].text : ""
        };
    }
